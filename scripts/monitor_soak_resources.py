#!/usr/bin/env python
"""soak 渲染期间的资源/目录增长监控（后台运行，建议 UE 渲染期间在另一个终端跑）。
...
用法（仓库根目录，.venv）：
  uv run python scripts/monitor_soak_resources.py \
      --input G:/FutsalMOT_Dataset/episode_0001 \
      --interval 30 \
      --out soak_resources.csv
...
def main() -> int:
    ap = argparse.ArgumentParser(description="soak 渲染资源监控")
    ap.add_argument("--input", required=True, help="数据集输出根目录（G:/FutsalMOT_Dataset/episode_0001）")
