...
from grf_ue_bridge.config.paths import default_repo_root
...
def main(argv: object = None) -> int:
    parser = argparse.ArgumentParser(
        prog="grf-ue benchmark", description="后处理性能基准工具"
    )
    ...
    # 解析 cryptomatte 配置（不再读取根目录隐式 ue_import_config.json）
    mapping_dict = None
    ...
    if args.only in (None, "cryptomatte"):
        ...
        if (mapping_path is None or episode_path is None
            ...
            print("ERROR: cryptomatte 阶段需要显式 --mapping 与 --episode"
                  "（不再从根目录 ue_import_config.json 推断；可用 grf-ue task postprocess）")
            return 1
