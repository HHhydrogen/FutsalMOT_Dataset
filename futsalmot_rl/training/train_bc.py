...
def train_bc(
    ...
    Args:
        ...
        log_dir: Directory for logs and loss curve.
        video_dir: Directory for training videos (None = no videos).
        config: Training configuration dict.
        device: 'auto', 'cpu', or 'cuda'.
        video_callback: Optional callable(policy, epoch, video_dir) for video.
    ...
    train_dataset = DemoDataset(demo_index_path, split="train", train_ratio=cfg["train_split"], val_ratio=cfg["val_split"])
    val_dataset = DemoDataset(demo_index_path, split="val", train_ratio=cfg["train_split"], val_ratio=cfg["val_split"])
    ...
    policy.to(device)
    optimizer = torch.optim.Adam(policy.parameters(), lr=cfg["learning_rate"], weight_decay=cfg["weight_decay"])
    loss_fn = nn.MSELoss()
    ...
    summary: dict[str, Any] = {
        "config": cfg, "epochs": [],
        "best_val_loss": None, "best_epoch": None, "total_train_time_s": 0.0,
    }
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        for obs_batch, act_batch in train_loader:
            ...
            pred = policy.deterministic_action_tensor(obs_batch)
        ...
        log_entry = {"epoch": epoch, "train_loss": avg_train_loss, "val_loss": avg_val_loss}
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_epoch = epoch
            best_model_path = Path(model_out).with_name(Path(model_out).stem + "_best" + Path(model_out).suffix)
            save_policy(policy, best_model_path, config=cfg, metrics={
                "epoch": epoch, "val_loss": avg_val_loss, "train_loss": avg_train_loss, "best_val_loss": best_val_loss,
            })

        if epoch % 5 == 0 or epoch == 1 or epoch == cfg["epochs"]:
            print(f"  Epoch {epoch}/{cfg['epochs']}: train_loss={avg_train_loss:.6f} val_loss={avg_val_loss:.6f} {'(best)' if epoch == best_epoch else ''}")

        if video_callback is not None and video_dir is not None and epoch % cfg["eval_interval_epochs"] == 0:
            try:
                video_callback(policy, epoch, Path(video_dir))
