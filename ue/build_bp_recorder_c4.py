...
def _build_group(group):
    ...
    # exec 链：Entry→SetStart0→(L0 段)→SetEnd0→Sub→AddDur0→SetStart1→(L1 段)→SetEnd1→Sub→AddDur1
    # Sub 是纯函数（无 exec），exec 在 AddDur 之间
    entry_pin = fn_editor.find_graph_entry_pin()
    prev, prev_pin = None, None
    try:
        entry_pin.try_create_connection(fn_pins["setstart"][0]["execute"])
        _log("  [conn] Entry→SetStart0(exec): True")
        prev, prev_pin = fn_pins["setstart"][0], "then"
    except Exception as e:
        _log(f"  [conn] Entry→SetStart0 ERR: {type(e).__name__}")
    ...
    _set_value(pins["save"], "slotname", slot, "SaveToSlot")
