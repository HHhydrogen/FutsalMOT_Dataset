...
from grf_ue_bridge.config.paths import ENV_DATASET_ROOT, ENV_UE_PROJECT_ROOT


def _make_resolved(tmp_path: Path):
    repo = tmp_path / "repo"
    ds = tmp_path / "ds"
    ...
    rt = resolver.resolve_task(repo / "task.json", env={
        ENV_UE_PROJECT_ROOT: str(repo), ENV_DATASET_ROOT: str(ds),
    })
    return rt, repo


class TestResolvedTaskContract:
    def test_contains_ue_required_fields(self, tmp_path):
        rt, repo = _make_resolved(tmp_path)
    ...
    def test_load_resolved_rejects_bad_schema(self, tmp_path):
        rt, repo = _make_resolved(tmp_path)
    ...
    def test_load_resolved_rejects_bad_version(self, tmp_path):
        rt, repo = _make_resolved(tmp_path)
        path = resolver.save_resolved_task(rt, repo)
        data = json.loads(path.read_text(encoding="utf-8"))
        data["version"] = 99
    ...
    def test_sanitized_no_absolute(self, tmp_path):
        rt, repo = _make_resolved(tmp_path)
    ...
    def test_runtime_path_inside_gitignored(self, tmp_path):
        rt, repo = _make_resolved(tmp_path)
