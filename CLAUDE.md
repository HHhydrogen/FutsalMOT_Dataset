...
- `ue/pose_export.py`（UE 侧）— 逐帧读骨骼 transform → 世界 3D 关键点（米）→ 每相机 `pose_keypoints.jsonl`（可含可选遮挡 trace 的 `occluded` 标志）。**骨骼解析用 probe 方式**（逐候选骨读 world transform，非全零四元数即有效；不依赖列出骨骼名——`get_ref_skeleton` 在 UE 5.8 Python 不可用）；**遮挡 trace 默认关闭**（`occlusion_trace=false`：mask 判定已是主要遮挡信号，61 万次射线会拖慢大任务；显式开启时全防御，API 缺失自动跳过）。
...
- `src/grf_ue_bridge/debug.py`（P1 纯 Python，pillow+numpy+cv2）— 全量 debug 可视化：`postprocess.debug.enabled` 或 `grf-ue debug` 时对每 camera 渲染 bbox overlay / 彩色 mask / pose 关节点（只画点+骨架连线，无文字）三套图集，并把三套图集各拼接为 mp4（`video_bbox/mask/pose.mp4`）；`annotate-overlay`/`make-video` 复用其实现。
