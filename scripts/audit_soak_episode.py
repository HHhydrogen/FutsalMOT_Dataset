...
并用退出码标识是否通过。检查维度：
  ...
  - camera.json 标定合法性（分辨率一致、内参有限且为正、外参有限、四相机不重复）
  - render_summary.json 状态与每相机 ok
  - 可选：进程内运行 validate-annotations（quick/full）

用法（仓库根目录，.venv）：
  uv run python scripts/audit_soak_episode.py \
      --input G:/FutsalMOT_Dataset/episode_0001 \
      --expected-cameras 4 \
      --expected-frames-per-camera 300 \
      --episode outputs/episode_0001 \
...
sys.path.insert(0, str(REPO_ROOT / "ue"))
